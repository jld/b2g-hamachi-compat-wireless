...
#define AR6003_REGCODE_ADDRESS_OFFSET 12 

#ifdef __CUSTOMIZE_WIFI_CHANNEL11__
static u16 regcode = 0x8348; /* setting USA for just support channel 1-11 */ 
#else
static u16 regcode = 0x809C; /* setting USA for just support channel 1-13 */
#endif

static void ath6kl_mangle_regcode(struct ath6kl *ar, u16 regcode) 
{ 
    u8 *ptr_regcode; 

    switch (ar->target_type) { 
        case TARGET_TYPE_AR6003: 
        ptr_regcode = ar->fw_board + AR6003_REGCODE_ADDRESS_OFFSET; 
        break; 
        default: 
        ath6kl_err("Invalid Target Type\n"); 
        return; 
    ...
    ath6kl_dbg(ATH6KL_DBG_BOOT, 
    "REGCODE from EEPROM %02X%02X\n", 
    ptr_regcode[1], ptr_regcode[0]); 

    ptr_regcode[0] = (u8)(regcode & 0xFF); 
    ptr_regcode[1] = (u8)((regcode >> 8) & 0xFF); 
...
int ath6kl_core_init(struct ath6kl *ar)
{
	...
	if (regcode!=0) { 
        	ath6kl_mangle_regcode(ar, regcode);
	}
